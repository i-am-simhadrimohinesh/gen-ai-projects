...
class LearningService:
    ...
    def get_topic_notes_markdown(
        ...
        if learning_content is None:
            raise ValueError(
                "Notes have not been generated for this topic yet."
            )
        ...
        filename = (
            f"{topic.title.replace(' ', '_').lower()}.md"
        )
    ...
    def get_weak_topics(self, journey_id: int) -> list[dict]:
        weak_topics = (
            ...
            .filter(
                ...
                LearnerKnowledge.score < WEAK_TOPIC_THRESHOLD,
            )
            .order_by(LearnerKnowledge.score.asc())
            .all()
        )
