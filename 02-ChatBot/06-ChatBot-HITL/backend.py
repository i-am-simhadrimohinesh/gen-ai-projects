...
def purchase_stocks(symbol:str,quantity:int)->dict:
    '''Simulation : Purchase of stocks based on symbol and quantity
    Human In the Loop : 
    If user approves then only stocks will be purchase
    '''
    decision = interrupt({
        'type' : 'approve',
        'reason' : f"Purchase {quantity} shares of {symbol}",
        'question' : f"Do you want to purchase {quantity} shares of {symbol}?",
        'instruction' : 'Approve or reject this purchase.'
    })
    if decision['result'] == "approved":
        return {
            'status' : 'success',
            'message' : 'successful bought the stocks',
            'symbol' : symbol,
            'quantity' : quantity
        }
    else : 
        return {
            'status' : 'fail',
            'message' : 'Purchase declined by the user',
            'symbol' : symbol,
            'quantity' : quantity
        }
...
tools = [search_tool,calculator,rag_retriever,purchase_stocks]
